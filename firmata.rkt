...
;; This is for measuring distance using an ultrasonic sensor;
;; we store the last reading in mm
(define SONAR_DISTANCE 0)

;; Just return the distance
(define (getSonarDistance) SONAR_DISTANCE)
    ...
    ((= data START-SYSEX)
     ...
     (cond ( (= (first rec-msg) MYRTLE-DATA) 
             (cond ( (= (second rec-msg) ENCODER-REPLY)
                     ...
                     ) ;; closes cond to parse an encoder reply 
                   ( (= (second rec-msg) DISTANCE-REQUEST)
                     ;; FIXME: hardcoded message size
                     (cond ( (and (> (length rec-msg) 3) (< (length rec-msg) 20))
                            (define msgBody0 (list-ref rec-msg 3))
                            (define msgBody1 (list-ref rec-msg 4))
                            (define msgBody2 (list-ref rec-msg 4))
                            
                            (define distance (+ (arithmetic-shift msgBody1 7) msgBody0))
                            (set! SONAR_DISTANCE distance)
                            )
                           (else (printf "WARNING: I receievd a sysex message of strange size: ~a\n" rec-msg))
                           )
                     ) ;; closes a distance request reply 
                   ) ;; closes cond to parse a SYSEX message
             )
